...
public class DebitorServiceTest {
    ...
    @Mock
    private DebitorRepository repository;

    @InjectMocks
    private DebitorService service;
    ...
    void save() {
        ...
        // Arrange
        final var obj = Debitor.builder().build();
        when(repository.save(any(Debitor.class))).thenReturn(obj);

        // Act
        final var actual = service.create(new Debitor());

        // Assert
        assertThat(actual).usingRecursiveComparison().isEqualTo(obj);
        verify(repository, times(1)).save(any(Debitor.class));
    ...
    void findById() {
        ...
        // Arrange
        final var obj = Optional.of(Debitor.builder().build());
        when(repository.findById(anyLong())).thenReturn((Optional<Debitor>) obj);
    ...
    void findAll() {
        ...
        // Arrange
        when(repository.findAll()).thenReturn(List.of(new Debitor(), new Debitor()));
    ...
    void update() {
        ...
        // Arrange
        final var obj = Debitor.builder().id(1L).build();
        final var updated = Debitor.builder().id(1L).build();
