...
@ActiveProfiles("dev")
public class PaidServiceTest {

    @Mock
    private PaidRepository repository;
    ...
    void save() {
        ...
        // Arrange
        final var obj = Paid.builder().type("myName1").build();
        when(repository.save(any(Paid.class))).thenReturn(obj);

        // Act
        final var actual = service.create(new Paid());

        // Assert
        assertThat(actual).usingRecursiveComparison().isEqualTo(obj);
        verify(repository, times(1)).save(any(Paid.class));
    ...
    void findById() {
        ...
        // Arrange
        final var obj = Optional.of(Paid.builder().type("myName1").build());
        when(repository.findById(anyLong())).thenReturn((Optional<Paid>) obj);
    ...
    void findAll() {
        ...
        // Arrange
        when(repository.findAll()).thenReturn(List.of(new Paid(), new Paid()));
    ...
    void update() {
        ...
        // Arrange
        final var obj = Paid.builder().id(1L).type("myName1").build();
        final var updated = Paid.builder().id(1L).type("myName1updated").build();
